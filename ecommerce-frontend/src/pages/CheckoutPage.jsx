...
const CheckoutPage = () => {
  ...
  const [fulfillmentMode, setFulfillmentMode] = useState('delivery'); // 'delivery' or 'pickup'
  ...
  const handleFulfillmentModeChange = (mode) => {
    ...
    if (mode === 'pickup') {
      ...
      setDeliveryCharge(0);
      setPaymentMethod('bank_transfer'); // Default to bank transfer or Pay at Counter
    } else {
  ...
  const handlePlaceOrder = () => {
    ...
    const orderData = {
      ...
      deliveryLocation: fulfillmentMode === 'pickup' ? 'Store Pickup' : (selectedDeliverySetting?.type || null),
      deliveryType: fulfillmentMode === 'pickup' ? 'store_pickup' : (selectedDeliverySetting?.type || null),
    ...
    <>
                ...
                <div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <button
                          ...
                          className={`flex flex-col items-center gap-3 p-4 rounded-xl border-2 transition-all ${fulfillmentMode === 'delivery'
                            ? 'border-primary-600 bg-primary-50 text-primary-600'
                            : 'border-gray-200 bg-white text-gray-400 hover:border-gray-300'
                            }`}
                        >
                          <ShoppingBag className="w-6 h-6" />
                          <span className="text-xs font-bold uppercase tracking-wider">Home Delivery</span>
                        </button>
                        <button
                          onClick={() => handleFulfillmentModeChange('pickup')}
                          className={`flex flex-col items-center gap-3 p-4 rounded-xl border-2 transition-all ${fulfillmentMode === 'pickup'
                            ? 'border-primary-600 bg-primary-50 text-primary-600'
                            : 'border-gray-200 bg-white text-gray-400 hover:border-gray-300'
                            }`}
                        >
                          <Store className="w-6 h-6" />
                          <span className="text-xs font-bold uppercase tracking-wider">Store Pickup</span>
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">
                          {fulfillmentMode === 'pickup' ? 'Billing Address *' : 'Delivery Address *'}
                        </label>
                        <textarea
                          name="address"
                          ...
                          rows={3}
                          className="input-field"
                          placeholder={fulfillmentMode === 'pickup' ? "Enter your billing address" : "Enter your complete delivery address"}
                          required
                        />
                      </div>

                      {fulfillmentMode === 'delivery' && activeDeliverySettings.length > 0 && (
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-2">
                            Delivery Options
                          </label>
                            ...
                            <div className="space-y-3">
                              {activeDeliverySettings.map((setting) => {
                                ...
                                return (
                                  <label
                                    key={setting.type}
                                    className={`block p-4 border-2 rounded-lg cursor-pointer transition-all ${selectedDeliveryType === setting.type ? 'border-primary-600 bg-primary-50' : 'border-gray-200 hover:border-gray-300'
                                      }`}
                                  >
                                    <div className="flex items-start">
                                      ...
                                      <div className="flex-1">
                                        <div className="flex justify-between items-start">
                                          ...
                                          <p className="font-semibold text-primary-600">Rs. {charge.toFixed(2)}</p>
                                        </div>
                                      </div>
                                    </div>
                                  </label>
                          ...
                          {deliveryCharge > 0 && selectedDeliveryType && (
                            <div className="mt-4 p-4 bg-primary-50 border border-primary-200 rounded-lg">
                              <p className="font-semibold text-primary-900 mb-2 text-sm">Delivery Charge Breakdown</p>
                              {(() => {
                                  ...
                                  return (
                                    <div className="space-y-1 text-xs">
                                      <div className="flex justify-between">
                                        ...
                                        <span className="font-bold">Rs. {baseAmount.toFixed(2)}</span>
                                      </div>
                                      {extraWeight > 0 && (
                                        <div className="flex justify-between">
                                          <span className="text-gray-700">Extra weight ({extraWeight.toFixed(2)} kg × Rs. {perKgAmount.toFixed(2)}):</span>
                                          <span className="font-bold">Rs. {(extraWeight * perKgAmount).toFixed(2)}</span>
                                        </div>
                                      )}
                                      <div className="pt-2 mt-2 border-t border-primary-300 flex justify-between font-black text-sm">
                                        <span className="text-primary-900">Total Delivery Charge:</span>
                                        <span className="text-primary-600">Rs. {deliveryCharge.toFixed(2)}</span>
                                      </div>
                                    </div>
                                  );
                                }
                                return (
                                  <div className="flex justify-between text-xs">
                                    ...
                                    <span className="font-bold text-primary-600">Rs. {deliveryCharge.toFixed(2)}</span>
                                  </div>
                                );
                              })()}
                            </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-6 sticky top-24 overflow-hidden">
              ...
              <div className="border-t border-gray-100 pt-5 space-y-3 relative">
                ...
                <div className="flex justify-between text-xs">
                  <span className="text-gray-500 font-medium">{fulfillmentMode === 'pickup' ? 'Fulfillment' : 'Delivery'}</span>
                  {fulfillmentMode === 'pickup' ? (
                    <span className="text-green-500 font-bold uppercase tracking-wider text-right">Store Pickup</span>
                  ) : selectedDeliveryType ? (
                    <span className="text-primary-600 font-bold italic">
                      LKR {deliveryCharge.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                    </span>
                  ) : (
                    <span className="text-gray-400 font-bold italic text-right">Pending Selection</span>
                  )}
                </div>
                  ...
                  <div className="flex justify-between items-baseline mb-1">
                    ...
                    <div className="text-right">
                      <span className="text-xl font-black text-gray-900 block leading-none">
                        LKR {(totalAmount + (fulfillmentMode === 'pickup' ? 0 : deliveryCharge)).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                      </span>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-xl border-t border-gray-100 p-4 z-50 lg:hidden flex items-center justify-between gap-4 shadow-[0_-8px_30px_rgb(0,0,0,0.05)]">
        <div className="flex flex-col">
          ...
          <span className="text-lg font-black text-gray-900 leading-none">LKR {(totalAmount + (fulfillmentMode === 'pickup' ? 0 : deliveryCharge)).toFixed(2)}</span>
        </div>
